...
public class GoogleAuthorizationCodeAccessTokenProvider extends AuthorizationCodeAccessTokenProvider
{
		...
		AccessTokenRequest request)
	{
		...
		// if (request.getStateKey() != null)
		// {
		// // The token endpoint has no use for the state so we don't send it back, but
		// // we are using it
		// // for CSRF detection client side...
		// if (preservedState == null)
		// {
		// throw new InvalidRequestException(
		// "Possible CSRF detected - state parameter was present but no state could be
		// found");
		// }
		// }

		// Extracting the redirect URI from a saved request should ignore the current URI,
		// so it's not simply a call to
		...
		AccessTokenRequest request)
	{
		...
		if (request.containsKey(OAuth2Utils.USER_OAUTH_APPROVAL)
			&& "consent".equals(request.getFirst(OAuth2Utils.USER_OAUTH_APPROVAL)))
		{
			requestParameters.put("prompt", request.getFirst(OAuth2Utils.USER_OAUTH_APPROVAL));
			request.remove(OAuth2Utils.USER_OAUTH_APPROVAL);
		}
