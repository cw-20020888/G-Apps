...
(function($,undefined){$.widget("mobile.date",{options:{defaultDate:null,appendText:"",buttonText:"...",buttonImage:"",buttonImageOnly:false,hideIfNoPrevNext:false,navigationAsDateFormat:false,gotoCurrent:false,changeMonth:false,changeYear:false,yearRange:"c-10:c+10",showOtherMonths:false,selectOtherMonths:false,showWeek:false,calculateWeek:this.iso8601Week,shortYearCutoff:"+10",minDate:null,maxDate:null,beforeShowDay:null,onSelect:function(text,object){var self=this;setTimeout(function(){if(!object.settings.inline){$(object.input).date("addMobileStyle");}else{$(object.settings.altField).date("addMobileStyle");}},0);},onChangeMonthYear:function(month,year,object){var self=this;setTimeout(function(){if(!object.settings.inline){$(object.input).date("addMobileStyle");}else{$(object.settings.altField).date("addMobileStyle");}},0);},beforeShow:function(element){var self=this;setTimeout(function(){$(element).data("mobileDate").addMobileStyle();},0);},numberOfMonths:1,showCurrentAtPos:0,stepMonths:1,stepBigMonths:12,altField:"",altFormat:"",constrainInput:true,showButtonPanel:false,autoSize:false,disabled:false,inline:false},_create:function(){var calendar,interval,that=this;if(this.options.inline){this.options.altField=this.element;calendar=$("<div>").datepicker(this.options);this.element.parent().after(calendar);}else{this.element.datepicker(this.options);calendar=this.element.datepicker("widget");}
this.calendar=calendar;this.baseWidget=(!this.options.inline)?this.element:this.calendar;this._on({"change":function(){if(this.options.inline){this.calendar.datepicker("setDate",this.element.val());}
